...
let bank_type = if is_first {
    ...
    // 20% Investment, 10% Universal. Cooperative banks (SKOK) handle
...
**Note:** `compute_deltas` must receive `peasant_population`, `furloughed_total`, and `gdp_per_capita` as parameters. Update the function signature and call site accordingly.
...
- **Save compatibility:** Adding fields to `TelemetrySample` with `#[serde(default)]` ensures old saves load correctly (Rule 10 allows save breaks in alpha, but `#[serde(default)]` is cleaner here since the fields are purely additive).
