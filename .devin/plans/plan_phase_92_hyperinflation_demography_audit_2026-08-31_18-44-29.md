...
### Pillar 5: Gendered Names, Visual Metadata & Extended Dynasties
...
**Root cause (visual metadata gap):** The `Vip` struct (`vip_registry.rs:312-376`) has a `gender` field but no `portrait_seed` or visual metadata. The `VipDossierRow` DTO (`snapshot.rs:1684-1697`) and `DynastyMemberRow` DTO (`snapshot.rs:2465-2479`) both LACK the `gender` field entirely. This means:
- The VIP explorer list cannot display gender-appropriate visual indicators.
- The Dynasty Tree cannot distinguish male/female members visually.
- If a future avatar/portrait system is added, it would have no gender-correct seed to generate from.
...
3. **Add `portrait_seed` to the `Vip` struct:**
   - Add `pub portrait_seed: String` to `Vip` (`vip_registry.rs`). Mark with `#[serde(default)]`.
   - The seed is computed at VIP creation as: `format!("{}-{}-{}", cultural_group, gender, full_name)`. This incorporates gender into the visual seed, ensuring any future avatar generator (procedural or asset-based) produces gender-appropriate visuals.
   - Set `portrait_seed` in all VIP creation sites (genesis, dynasty initialization, dynasty turn processing, CEO generation). For existing VIPs loaded from old saves, `#[serde(default)]` produces an empty string, which the frontend can handle as a fallback.

4. **Add `gender` to `VipDossierRow` and `DynastyMemberRow` DTOs:**
   - Add `pub gender: String` to `VipDossierRow` (`snapshot.rs:1684`). Populate from `Vip.gender` during DTO construction.
   - Add `pub gender: String` to `DynastyMemberRow` (`snapshot.rs:2465`). Populate from the VIP registry lookup during DTO construction.
   - Add `pub portrait_seed: String` to both DTOs. Populate from `Vip.portrait_seed`.
   - This enables the frontend to render gender-appropriate visual indicators (icons, colors, avatar placeholders) and provides the seed for future portrait generation.

5. **Frontend visual indicators (minimal, no new asset system):**
   - In `VipsPage.tsx`, add a gender icon/badge next to each VIP row (e.g., ♂/♀ or color-coded badge).
   - In `DynastyTree.tsx`, add a gender indicator to each dynasty node (e.g., 👑 for male monarch, 👑♀ for female monarch, ⚭ for consort with gender color).
   - These are text-based indicators, not a full portrait system. The `portrait_seed` field is available for future procedural avatar generation but is not rendered as an image in this phase.

6. **Extend royal dynasty initialization:**
   ...
   - Set `portrait_seed` for all new dynasty members using the gender-aware seed formula.
...
### Step 10: Pillar 5c — Portrait Seed & Gender in DTOs (vip_registry.rs + snapshot.rs + frontend)

**File:** `state/src/politics/vip_registry.rs`
- Add `pub portrait_seed: String` to the `Vip` struct (~line 376). Mark with `#[serde(default)]`.
- The seed is computed at VIP creation as: `format!("{}-{}-{}", cultural_group, gender, full_name)`. This incorporates gender into the visual seed.
- Set `portrait_seed` in all VIP creation sites in `turn.rs` and `succession.rs` during Step 8-9.

**File:** `state/src/ui/snapshot.rs`
- Add `pub gender: String` and `pub portrait_seed: String` to `VipDossierRow` (~line 1684). Populate from `Vip.gender` and `Vip.portrait_seed` during DTO construction.
- Add `pub gender: String` and `pub portrait_seed: String` to `DynastyMemberRow` (~line 2465). Populate from the VIP registry lookup during DTO construction.

**File:** `src/pages/VipsPage.tsx`
- Add a gender indicator (badge or icon) next to each VIP row in the list.
- Display the gender field from the `VipDossierRow` DTO.

**File:** `src/components/DynastyTree.tsx`
- Add a gender indicator to each dynasty node (e.g., color-coded badge or icon).
- Display the gender field from the `DynastyMemberRow` DTO.
...
- `state/src/politics/turn.rs` — Pillar 5a (consort gender) + Pillar 5b (extended dynasty) + Pillar 5c (portrait_seed at creation)
- `state/src/politics/succession.rs` — Pillar 5a (spouse gender) + Pillar 5b (Uncle/Aunt relation) + Pillar 5c (portrait_seed at creation)
- `state/src/politics/vip_registry.rs` — Pillar 5c (add portrait_seed to Vip struct)
- `state/src/ui/snapshot.rs` — Pillar 5c (add gender + portrait_seed to VipDossierRow and DynastyMemberRow)
- `src/pages/VipsPage.tsx` — Pillar 5c (gender indicator in VIP list)
- `src/components/DynastyTree.tsx` — Pillar 5c (gender indicator in dynasty tree)
...
- [ ] Manual verification: Generate a new world and verify:
  ...
  - VIP portrait_seed incorporates gender (e.g., "slavic-F-Anna Kowalska")
  - VipDossierRow and DynastyMemberRow DTOs include gender field
  - Frontend VIP list and Dynasty Tree display gender indicators
...
| Asymmetric Information | PASS | The `portrait_seed` field is a deterministic string derived from public VIP attributes (culture, gender, name) — it contains no classified or hidden data. The `gender` field added to `VipDossierRow` and `DynastyMemberRow` is already public on `VipDossier`. Transient wage fields are `#[serde(skip)]` and never serialized. No hidden data is sent to the frontend. The portrait_seed enables future avatar generation without exposing backend secrets. |
| Full-Stack Accountability | PASS | Financial history fix (Pillar 3) ensures existing DTO fields are correctly populated. `CompaniesPage.tsx` already displays these fields. Pillar 5c adds `gender` and `portrait_seed` to `VipDossierRow` and `DynastyMemberRow` DTOs, and updates `VipsPage.tsx` and `DynastyTree.tsx` to display gender indicators. Vein diversity visible through existing geological/exploration UI. Company size distribution is visible through existing company list/detail UI. All new backend data has corresponding frontend visibility. |
