...
fn test_compound_rate_conversion_is_not_simple_division() {
    ...
    assert!(r_turn_compound < r_turn_simple,
        "Compound rate {} should be < simple rate {}", r_turn_compound, r_turn_simple);
    // But they should be close for small rates
    assert!((r_turn_compound - r_turn_simple).abs() < 0.001,
    ...
    assert!((compounded - r_annual).abs() < 1e-10,
        "24 turns of compound rate should equal annual rate: {} vs {}", compounded, r_annual);
}
...
fn test_compound_rate_conversion_high_rate() {
    ...
    let compounded: f64 = (1.0_f64 + r_turn_compound).powi(24) - 1.0;
    assert!((compounded - r_annual).abs() < 1e-10,
        "24 turns should compound to annual: {} vs {}", compounded, r_annual);
}
...
fn test_meat_and_fruit_in_subsistence_baskets() {
    ...
    if let Some(basket) = registry.get("FreePeasant") {
        let subsistence = basket.tiers.get(&consumption_registry::NeedTier::Subsistence);
...
fn test_draft_animal_breeding_method_exists() {
    ...
    let agriculture = registry.get("agriculture")
        .expect("agriculture sector should exist");
    let has_draft = agriculture.iter_all().any(|m| {
        m.outputs.contains_key(&Commodity::DraftAnimals)
    });
    assert!(has_draft, "Agriculture should have a method that outputs DraftAnimals");
}

#[test]
fn test_cartridge_manufacturing_method_exists() {
    let registry = default_production_methods();
    let armaments = registry.get("armaments_industry")
        .expect("armaments_industry sector should exist");
    let has_cartridge = armaments.iter_all().any(|m| {
        m.outputs.contains_key(&Commodity::Ammunition) && m.year <= 1880
    });
    assert!(has_cartridge,
...
fn test_ammunition_available_at_1925_start() {
    ...
    let available = armaments.iter_all().any(|m| {
        m.year <= 1925 && m.outputs.contains_key(&Commodity::Ammunition)
    });
    assert!(available,
...
fn test_calorific_value_ordering() {
    ...
    assert!(Commodity::Uranium.calorific_value_mj_per_unit() > Commodity::NaturalGas.calorific_value_mj_per_unit());
    // NaturalGas > Oil > HardCoal > BrownCoal
    assert!(Commodity::NaturalGas.calorific_value_mj_per_unit() > Commodity::Oil.calorific_value_mj_per_unit());
    assert!(Commodity::Oil.calorific_value_mj_per_unit() > Commodity::HardCoal.calorific_value_mj_per_unit());
    assert!(Commodity::HardCoal.calorific_value_mj_per_unit() > Commodity::BrownCoal.calorific_value_mj_per_unit());
}
...
fn test_energy_methods_have_thermal_efficiency() {
    ...
    let coal_boiler = energy.iter_all().find(|m| {
        m.inputs.contains_key(&Commodity::HardCoal) && m.outputs.contains_key(&Commodity::Energy)
    }).expect("Coal-Fired Boilers method should exist");
    assert!(coal_boiler.thermal_efficiency > 0.0,
        "Coal-Fired Boilers should have thermal_efficiency > 0, got {}",
        coal_boiler.thermal_efficiency);
    assert!(coal_boiler.thermal_efficiency <= 1.0,
...
fn test_combined_cycle_has_higher_efficiency_than_coal_boiler() {
    let registry = default_production_methods();
    let energy = registry.get("energy").unwrap();
    let coal = energy.iter_all().find(|m| {
        m.thermal_efficiency > 0.0 && m.inputs.contains_key(&Commodity::HardCoal)
    }).expect("Coal-Fired Boilers should exist");
    let combined = energy.iter_all().find(|m| {
        m.thermal_efficiency > 0.0 && m.inputs.contains_key(&Commodity::NaturalGas) &&
        m.outputs.contains_key(&Commodity::Heat)
    }).expect("Combined Cycle Plant should exist");
    assert!(combined.thermal_efficiency > coal.thermal_efficiency,
...
fn test_price_substitution_matrix_exists() {
    ...
    assert!(matrix.contains_key(&Commodity::Meat),
        "Price substitution matrix should have Meat as a primary");
    let meat_subs = matrix.get(&Commodity::Meat).unwrap();
    assert!(!meat_subs.is_empty(), "Meat should have at least one substitute");
    // Check elasticity coefficients are in the approved 0.7-0.9 range
    for sub in meat_subs {
        assert!(sub.elasticity_coefficient >= 0.7 && sub.elasticity_coefficient <= 0.9,
            "Elasticity coefficient {} should be in [0.7, 0.9]", sub.elasticity_coefficient);
        assert!(sub.max_substitution > 0.0 && sub.max_substitution <= 1.0,
            "Max substitution {} should be in (0, 1]", sub.max_substitution);
    }
}
...
fn test_homeless_class_zero_housing_demand() {
    ...
    let rate = sim_engine::economy::trade::retail::test_class_housing_possession_rate(
        &housing_buildings, "region_1", "Worker", 1000,
    );
    assert_eq!(rate, 0.0, "Homeless class should have 0 housing possession rate");
}
...
fn test_housed_class_positive_housing_demand() {
    ...
    let rate = sim_engine::economy::trade::retail::test_class_housing_possession_rate(
        &housing, "region_1", "FreePeasant", 1000,
    );
    // 200 slots × 4 household_size = 800 people housed out of 1000
    assert!(rate > 0.0, "Housed class should have positive housing rate, got {}", rate);
    assert!((rate - 0.8).abs() < 0.01, "Rate should be ~0.8 (800/1000), got {}", rate);
}
...
fn test_build_consumer_demand_with_substitution() {
    ...
    let meat_demand = demand.total_demand.get(&Commodity::Meat).copied().unwrap_or(0.0);
    let cereal_demand = demand.total_demand.get(&Commodity::Cereal).copied().unwrap_or(0.0);
    ...
    assert!(total_food >= 0.0, "Total food demand should be non-negative");
}
