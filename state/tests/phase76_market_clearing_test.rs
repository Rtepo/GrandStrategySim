...
use sim_engine::economy::market::market_history::{
    get_reference_price, update_vwap, MarketHistory,
};
use sim_engine::economy::market::order_book::{match_orders, Ask, Bid, OrderBook};
...
fn crossing_orders_produce_trades() {
    ...
    assert!(
        !book.trades.is_empty(),
        "Crossing orders should produce trades"
    );
...
fn non_crossing_orders_produce_no_trades() {
    ...
    assert!(
        book.trades.is_empty(),
        "Non-crossing orders should produce no trades"
    );
}
...
fn executed_trades_produce_positive_vwap() {
    ...
    assert!(
        vwap > 0.0,
        "VWAP should be positive after trades, got {}",
        vwap
    );
    assert!(
        last > 0.0,
        "Last trade price should be positive, got {}",
        last
    );
    // VWAP = (50*100 + 30*110) / (50+30) = 8300/80 = 103.75
    assert!(
        (vwap - 103.75).abs() < 0.01,
        "VWAP should be 103.75, got {}",
        vwap
    );
    // update_vwap sets last_trade_price to the VWAP (not the last individual trade price)
    assert!(
        (last - 103.75).abs() < 0.01,
        "Last trade price should be VWAP 103.75, got {}",
        last
    );
}
...
fn no_trade_commodity_uses_base_price_fallback() {
    ...
    assert!(
        (ref_price.unwrap() - 100.0).abs() < 0.01,
        "Fallback price should be 100.0"
    );
}
...
fn reference_price_fallback_chain_priority() {
    ...
    assert!(
        (ref_price - 105.0).abs() < 0.01,
        "VWAP should take priority: expected 105.0, got {}",
        ref_price
    );
    ...
    let ref_price = get_reference_price(&Commodity::Cereal, &history).unwrap();
    assert!(
        (ref_price - 120.0).abs() < 0.01,
        "Last trade should be used: expected 120.0, got {}",
        ref_price
    );
    ...
    let ref_price = get_reference_price(&Commodity::Cereal, &history).unwrap();
    assert!(
        (ref_price - 100.0).abs() < 0.01,
        "Base price should be used: expected 100.0, got {}",
        ref_price
    );
}
...
fn bootstrap_condition_detected() {
    ...
    let is_bootstrap = history.vwap_per_commodity.is_empty() && history.last_trade_price.is_empty();
    assert!(
        is_bootstrap,
        "Empty VWAP and last_trade should indicate bootstrap"
    );
