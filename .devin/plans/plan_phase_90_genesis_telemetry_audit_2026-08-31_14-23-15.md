...
#### Step 2.2: Update CompanyFinancialRecord DTO to expose wage_expense
**File:** `state/src/ui/snapshot.rs`  
**Location:** `CompanyFinancialRecord` struct (line ~1780)  
**Action:** Add `wage_expense` as a distinct field so the frontend can display it separately from other operating costs.
...
#### Step 2.3: Update compute_financial_summary to parse and propagate wage_expense
**File:** `state/src/ui/snapshot.rs`  
**Location:** `compute_financial_summary` function (line ~3269-3301)  
**Action:** The `parse_record` closure must read the new `wage_expense` field from the JSON. The `aggregate` closure must sum and average it. The `expenses` calculation remains `operating_costs + interest + taxes` (where `operating_costs` already includes `wage_expense` per Step 2.1), ensuring `net_profit = income - expenses` holds exactly.
...
let parse_record = |v: &serde_json::Value| -> CompanyFinancialRecord {
    ...
    CompanyFinancialRecord { income, expenses, net_profit, wage_expense }
};
...
**Invariant:** `net_profit == income - expenses` exactly, because `expenses = operating_costs + interest + taxes` and `operating_costs` already includes `wage_expense` (per Step 2.1). The `wage_expense` field is a transparency subset of `expenses`, not an additional deduction.

**Explicit initializer audit (Phase 89 lesson):**
- `CompanyFinancialRecord {` — two explicit initializers in `compute_financial_summary` (`parse_record` at line ~3280, `aggregate` at line ~3296). Both are covered by Step 2.3. The struct derives `Default`, so `CompanyFinancialRecord::default()` (line 3289) still compiles.

#### Step 2.4: Update CompaniesPage.tsx to display Wage Expense
**File:** `src/pages/CompaniesPage.tsx`  
**Location:** Financial History table (line ~187-216)  
**Action:** Add a "Wages" column between "Expenses" and "Net" to show the wage expense transparency subset. This makes it immediately clear when a company's losses are driven by wage obligations.
...
<tbody>
  ...
  {/* Same pattern for last_quarter and last_year */}
</tbody>
```

#### Step 2.5: Regenerate TypeScript types
**File:** `src/types/api.ts` (auto-generated)  
**Action:** Run `npm run build` or the `ts_rs` export step to regenerate `CompanyFinancialRecord` with the new `wage_expense` field. The generated type will become:
```typescript
export type CompanyFinancialRecord = { income: number, expenses: number, net_profit: number, wage_expense: number, };
```
...
- `state/src/corporate/manager.rs` — Pillar 2 (accrual wage expense in financial history JSON)
- `state/src/ui/snapshot.rs` — Pillar 2 (CompanyFinancialRecord DTO, compute_financial_summary), Pillar 5 (TelemetryDeltas fields, compute_deltas)
...
- `src/pages/CompaniesPage.tsx` — Pillar 2 (Wage Expense column in financial history table)
- `src/types/api.ts` — Pillar 2 & 5 (auto-generated via ts_rs export)
...
- [ ] Manual: Verify CompaniesPage detail panel shows "Wages" column in Financial History table
- [ ] Manual: Verify `net_profit == income - expenses` invariant holds in CompanyFinancialRecord
...
| Full-Stack Accountability | PASS | Pillar 2: Full stack planned — `financial_history` JSON (manager.rs) → `CompanyFinancialRecord` DTO with `wage_expense` field (snapshot.rs) → `compute_financial_summary` parser (snapshot.rs) → `CompaniesPage.tsx` Wages column (frontend). Pillar 5: Full stack planned — `TelemetrySample` (storage) → `TelemetryDeltas` (computation) → `MacroIndicatorsResponse` (DTO) → `MacroPage.tsx` (UI). Pillar 3 bank types are visible via existing banking queries. Pillar 4 veins are visible via existing Resources tab. |
