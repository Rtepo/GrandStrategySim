...
mod tests {
    ...
    fn test_climate_matrix_populated() {
        ...
        assert!(
            config.climate_season_matrix.is_empty(),
        ...
        assert_eq!(
            config.climate_season_matrix.len(),
            28,
    ...
    fn test_climate_matrix_all_agriculture_multipliers_nonzero() {
        ...
        for climate in &climates {
            for season in &seasons {
                ...
                assert!(
                    mods.agriculture_multiplier >= 0.0,
    ...
    fn test_geological_sparsity_not_all_regions_have_coal() {
        ...
        use sim_engine::society::geography::{
            generate_geological_formations, reseed_resources_from_formations, Climate, Region,
        };
    ...
    fn test_geological_sparsity_some_regions_have_no_energy_minerals() {
        use rand::thread_rng;
        use sim_engine::society::geography::{
            generate_geological_formations, reseed_resources_from_formations, Climate,
        };
        use std::collections::HashMap;
        ...
        let regions_without_energy = regions
            .values()
            .filter(|r| !energy_keys.iter().any(|k| r.resources.contains_key(*k)))
            .count();

        assert!(
            regions_without_energy >= 1,
    ...
    fn test_tropical_crops_exist_in_registry() {
        ...
        assert!(
            registry.get("rice").is_some(),
            "Rice should be in the crop registry"
        );
        assert!(
            registry.get("sugarcane").is_some(),
            "Sugarcane should be in the crop registry"
        );
        assert!(
            registry.get("coffee").is_some(),
            "Coffee should be in the crop registry"
        );
        assert!(
            registry.get("tea").is_some(),
            "Tea should be in the crop registry"
        );
        assert!(
            registry.get("soybeans").is_some(),
            "Soybeans should be in the crop registry"
        );
    }
    ...
    fn test_rice_compatible_with_tropical() {
        ...
        assert!(
            rice.compatible_climates.contains(&ClimateProfile::Tropical),
            "Rice must be compatible with Tropical climate"
        );
        assert!(
            rice.compatible_climates.contains(&ClimateProfile::Coastal),
    ...
    fn test_sugarcane_tropical_plantation() {
        ...
        assert!(
            sugarcane
                .compatible_climates
                .contains(&ClimateProfile::Tropical),
    ...
    fn test_soybeans_dual_climate_compatibility() {
        ...
        assert!(
            soy.compatible_climates.contains(&ClimateProfile::Temperate),
            "Soybeans must be compatible with Temperate climate"
        );
        assert!(
            soy.compatible_climates.contains(&ClimateProfile::Tropical),
    ...
    fn test_tea_mountainous_tropical() {
        ...
        assert!(
            tea.compatible_climates.contains(&ClimateProfile::Tropical),
            "Tea must be compatible with Tropical climate"
        );
        assert!(
            tea.compatible_climates
                .contains(&ClimateProfile::Mountainous),
    ...
    fn test_climate_aware_crop_batches_tropical() {
        ...
        for &crop_id in tropical_crops {
            ...
            assert!(
                crop.is_some(),
                "Tropical crop {} must exist in registry",
                crop_id
            );
            if let Some(c) = crop {
                assert!(
                    c.compatible_climates.contains(&ClimateProfile::Tropical),
                    "Crop {} must be compatible with Tropical",
                    crop_id
                );
            }
        }
