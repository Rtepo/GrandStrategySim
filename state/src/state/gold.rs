...
impl GlobalGoldExchange {
    ...
    pub fn execute_trade(
        ...
        let currency_rate = currencies.get(&order.payment_currency)
        ...
        let buyer_balance = buyer_account.get_currency_balance(&order.payment_currency, domestic_currency);
        if buyer_balance < payment_amount {
            return Err(format!("Insufficient {} payment: have {}, need {}", order.payment_currency, buyer_balance, payment_amount));
        }
        ...
        if matches!(order.order_type, ForexOrderType::Buy) {
            ...
            buyer_account.debit_currency(&order.payment_currency, payment_amount, domestic_currency);
            *vaults.entry(order.entity_id.clone()).or_insert(0.0) += order.gold_amount;
            
            // Seller (pool): credit payment to fiat_reserves, debit gold
            *self.fiat_reserves.entry(order.payment_currency.clone()).or_insert(0.0) += payment_amount;
            ...
            *vaults.entry(order.entity_id.clone()).or_insert(0.0) -= order.gold_amount;
            ...
            let pool_fiat = self.fiat_reserves.get(&order.payment_currency).unwrap_or(&0.0);
            if *pool_fiat < payment_amount {
                return Err(format!("Gold pool insufficient {} reserves: have {}, need {}", order.payment_currency, pool_fiat, payment_amount));
            }
            
            // Debit pool fiat_reserves, credit seller
            *self.fiat_reserves.entry(order.payment_currency.clone()).or_insert(0.0) -= payment_amount;
            buyer_account.credit_currency(&order.payment_currency, payment_amount, domestic_currency);
    ...
    pub fn execute_cb_trade(
        ...
        if matches!(order.order_type, ForexOrderType::Buy) {
            ...
            if *available_fx < payment_amount {
                return Err(format!("Insufficient {} fx reserves: have {}, need {}", order.payment_currency, available_fx, payment_amount));
            }
