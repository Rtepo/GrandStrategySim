...
# ─── Helper: RAM throttle check (v3.1) ─────────────────────────────────────
# Checks system RAM usage. Returns 0 if below threshold, 1 if above.
# v3.1 Patch: bc-free — converts float to int via bash parameter expansion.
RAM_THROTTLE_PCT=85

check_ram_throttle() {
    local used_pct=0

    # Try PowerShell (Windows) first
    ...
    case "$used_int" in
        ''|*[!0-9]*) return 0 ;;  # Can't detect — proceed without throttling
    esac

    if [ "$used_int" -ge "$RAM_THROTTLE_PCT" ]; then
        echo "[$(date -u +%H:%M:%S)] RAM THROTTLE: System RAM at ${used_pct}% (threshold: ${RAM_THROTTLE_PCT}%)."
        return 1
    fi
...
run_cicd() {
    ...
    # v3.1: RAM throttle check before test execution (nextest is memory-intensive)
    local ram_retries=0
    while ! check_ram_throttle; do
        ram_retries=$((ram_retries + 1))
        if [ $ram_retries -ge 10 ]; then
            echo "[$(date -u +%H:%M:%S)] RAM THROTTLE: Exceeded 10 retries (5 min). Aborting CI/CD."
            git checkout main 2>/dev/null
            echo "TIMEOUT_FAILED" > "${log_prefix}_FAILED.txt"
            echo "TIMEOUT_FAILED"
            return 1
        fi
        echo "  Waiting 30s for memory to free (retry $ram_retries/10)..."
        sleep 30
    done
...
# ─── Helper: Aggressive GC — delete stale diagnostic files (v3.1) ──────────
# Scans the project root (maxdepth 1 — NEVER recursive) for stale diagnostic
# files generated by ad-hoc cargo/clippy/npm runs. Uses a single find command
# with concatenated -name patterns for efficiency.
cleanup_stale_diagnostics() {
    local deleted
    deleted=$(find "$HUB_DIR" -maxdepth 1 -type f \
        \( \
        -name "build_out*.txt" -o \
        -name "build_err*.txt" -o \
        -name "build_check*.txt" -o \
        -name "build_final*.txt" -o \
        -name "build_errors*.txt" -o \
        -name "clippy*.txt" -o \
        -name "clippy_output*.txt" -o \
        -name "*_test.txt" -o \
        -name "*_FAILED.txt" -o \
        -name "cicd_test.txt" -o \
        -name "cicd_output*.txt" -o \
        -name "npm_build*.txt" -o \
        -name "test_output*.txt" \
        \) -print -delete 2>/dev/null | wc -l)

    if [ "$deleted" -gt 0 ]; then
        echo "[$(date -u +%H:%M:%S)] GC: Deleted $deleted stale diagnostic files from project root."
    fi
}
...
while true; do
    ...
    # v3.1: Aggressive GC — clean stale diagnostic files every 50 cycles (~12.5 min)
